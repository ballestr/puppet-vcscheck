...
function vcsnotify() {
    ...
    if [ "$2" = "OK" ]; then
        ...
        curl -sS $CURLOPT -Fhost=$HOSTNAME -Fconf=$CONF -Fsecret=$NOTIFYSECRET -Fvcscheck=@$T $NOTIFYURL > /dev/null
        local R=$?
        rm $T
    else
        curl -sS $CURLOPT -Fhost=$HOSTNAME -Fconf=$CONF -Fsecret=$NOTIFYSECRET -Fvcscheck=@$MSGFILE $NOTIFYURL > /dev/null
        local R=$?
    fi
